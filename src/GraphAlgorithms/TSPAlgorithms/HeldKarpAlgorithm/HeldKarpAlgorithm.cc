...
TspResult HeldKarpAlgorithm::PerformHeldKarpAlgorithm(const Graph& graph) {
  ...
  const std::size_t number_of_vertices{graph.GetGraph().size()};
  GenerateSubSets(static_cast<int>(number_of_vertices) - 1);
  ...
  if (total_cost == std::numeric_limits<int>::max()) {
    ...
    result.vertices = GetPath(static_cast<int>(number_of_vertices));
  }

  return result;
}

void HeldKarpAlgorithm::GenerateSubSets(const int number_of_vertices) {
...
void HeldKarpAlgorithm::GenerateCombination(const std::vector<int>& numbers,
  ...
  std::set<int> set{result.begin(),
                    result.begin() + static_cast<int>(position)};
...
void HeldKarpAlgorithm::IterateThroughSubSets(
  ...
  for (const auto& set : sub_sets_) {
    for (int current_vertex{1}; current_vertex < number_of_vertices;
         ++current_vertex) {
...
int HeldKarpAlgorithm::GetCostSoFar(const std::set<int>& current_set,
                                    const int previous_vertex) {
  std::set<int> set_copy{current_set};
  set_copy.erase(previous_vertex);
  const Index current_index{previous_vertex, set_copy};
  const int cost{min_cost_map_.count(current_index)
...
int HeldKarpAlgorithm::GetTotalCost(
    const std::vector<std::vector<int>>& distances,
    const int number_of_vertices) {
...
std::vector<int> HeldKarpAlgorithm::GetPath(const int number_of_vertices) {
  std::vector<int> path{};
