...
int GetNextCity(const int num_vertices, std::vector<bool> visited,
                const std::vector<double>& probabilities, const double total_prob) {
...
TspResult ConstructAntTour(const Graph& graph, const std::vector<std::vector<double>>& pheromone) {
    ...
    for (int step = 0; step < num_vertices - 1; ++step) {
        ...
        for (int next_vertex = 0; next_vertex < num_vertices; ++next_vertex) {
            if (!visited[next_vertex]) {
                const double pheromone_level = std::pow(pheromone[current_vertex][next_vertex], kAlpha);
        ...
        const int next_vertex = GetNextCity(num_vertices, visited, probabilities, total_prob);
...
void UpdatePheromoneLevels(std::vector<std::vector<double>>& pheromone, const std::vector<TspResult>& ant_solutions) {
...
TspResult ColonyAlgorithm::SolveTravelingSalesmanProblem(const Graph& graph) {
    const int num_vertices = graph.GetSize();
    std::vector<std::vector<double>> pheromone(num_vertices, std::vector<double>(num_vertices));
