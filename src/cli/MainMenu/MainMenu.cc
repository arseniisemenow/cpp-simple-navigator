  ...
  while (true) {
    ...
    switch (c) {
      ...
      case 10:  // Enter key
        ...
        if (select_menu_items[selected_index].label ==
            select_menu_items[select_menu_items.size() - 1].label) {
          endwin();
          return 0;
        }
        clear();
        if (!graphs.empty()) {
          ...
          if (selected_index == s21::constants::cli::kMenuItem4Index) {
            ...
            int num_runs = 1;
            TSPComparisonMode(std::ref(graphs), selected_graph_index,
                              selected_algorithms, num_runs);
            continue;
          }
        }
