        ...
        switch (ch) {
            ...
            case 's':
                if (start_index_input.empty() || finish_index_input.empty()) {
                    ...
                    int shortest_path = graph_algorithms.GetShortestPathBetweenVertices(graph, start_index, finish_index);

                    mvprintw(0, 0, "Shortest Path Result:");
                    mvprintw(2, 2, "From %d to %d: %d", start_index, finish_index, shortest_path);
                    graph.Clear();
                    getch();
                }
                break;
