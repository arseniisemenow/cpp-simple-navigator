...
void ChooseFilePath(std::string& selected_path) {
  ...
  mvprintw(
      row - 2, 0,
      "Use arrow keys to navigate, press Enter to select, and Esc to cancel.");
  ...
  while (!done) {
    ...
    if (ch == KEY_UP || ch == KEY_DOWN) {
      ...
      std::sort(directories.begin(), directories.end());
      selected_index =
          std::min(selected_index, static_cast<int>(directories.size()) - 1);
    }
  }
...
void FindDatasetDirectoryRecursive(
    const fs::path& dir, int current_depth, int max_depth,
  ...
  for (const auto& entry : fs::directory_iterator(dir)) {
    if (entry.is_directory()) {
      FindDatasetDirectoryRecursive(entry.path(), current_depth + 1, max_depth,
                                    dataset_directory, txt_file_count);
...
std::string FindDatasetDirectory(const std::string& path, int max_depth) {
  std::unordered_map<std::string, int> txt_file_count;
  std::string dataset_directory;
  FindDatasetDirectoryRecursive(path, 0, max_depth, dataset_directory,
                                txt_file_count);
